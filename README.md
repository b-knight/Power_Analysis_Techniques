...
<div align="center"> 
<img src="https://github.com/b-knight/Power_Analysis_Techniques/blob/master/part_I_power_curve_estimation/lambda.png" align="middle" width="409" height="80" />
</div>
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; The results show method IX as producing the most consistent required sample size estimates. Across 100 iterations, the standard deviation of the sample size estimates came to approximately 3% of the mean of the sample size estimates (871/28,000). While this is not too bad an outcome, the notion that we can achieve better estimates by expending more compute was never in doubt. Of greater interest is that Scipy yields much more accurate curves as more points are added, and that the extent of this improvement tends to be far greater than that attained by increasing the number of simulations per point. For example, in Method VII we deployed 300% of the compute used in Method I (900 versus 300 simulations). Our reward for tripling our compute was to reduce by half the variance in the effective power estimates (7 percentage points worth of statistical power in Method VII versus 14 percentage points in Method I). In contrast, Methods V and VII performed very differently despite using a comparable amount of compute (1,000 and 900 simulations respectively). Across 100 iterations Methods V and VII yielded sample size estimates that on average, exceeded our target of 0.8 by 4 percentage points and fell short of our target by 3 percentage points respectively. However, Method V was far more consistent than model VII. For Method VII, the effective power estimates yielded from the recommended sample sizes had a standard deviation of 7 percentage points. For Method V, this figure is less than half at 3 percentage points.<br> 
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; The final, and most unintuitive result is that the estimates generated by interpolating the power curve tend to suffer from a positive bias. For Method VII, this bias is on the smaller side. While we sought a sample size that would allow us to detect the treatment effect 80% of the time, the resulting sample size recommendation was assessed as yielding approximately 82% power. Depending on one's circumstances, a discrepancy of this size might lengthen experimental run time by days - or weeks. This bias is likley tied to the fact that we commenced our exploration of the power curve with candidate sample sizes that ultimately proved to be less than half of what was ultimately required (10k observations versus 22~24k observations). When the experiment we are conducting the power analysis for has a signal-to-noise ratio is uncharacteristically small, our simulations may be at risk at overestimating the required sample size. We can only speculate whether this bias would trend towards the opposite direction if we need to run our power analysis for an experiment with an uncharacteristically large effect size.<br> 
